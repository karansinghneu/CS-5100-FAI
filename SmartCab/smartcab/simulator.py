            ...
            if status['violation'] == 0:  # Legal
                if status['waypoint'] == status['action']:  # Followed waypoint
                    ...
                    if status['light'] == 'red':  # Stuck at red light
                        ...
                        print("Agent idled at a green light with oncoming traffic. (rewarded {:.2f})".format(
                            status['reward']))
                else:  # Did not follow waypoint
                    print("Agent drove {} instead of {}. (rewarded {:.2f})".format(status['action'], status['waypoint'],
                                                                                   status['reward']))
            else:  # Illegal
                if status['violation'] == 1:  # Minor violation
                    print("Agent idled at a green light with no oncoming traffic. (rewarded {:.2f})".format(
                        status['reward']))
                elif status['violation'] == 2:  # Major violation
                    print("Agent attempted driving {} through a red light. (rewarded {:.2f})".format(status['action'],
                                                                                                     status['reward']))
